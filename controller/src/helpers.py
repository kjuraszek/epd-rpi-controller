...
def view_conditional(
    func: Callable[..., R]
) -> Callable[[VarArg(Any), KwArg(Any)], None]:
...
def wrap_titles(
    epd_width: int, epd_height: int, font: ImageFont.FreeTypeFont, titles: list[str]
) -> list[WrappedTitle]:
    ...
    for title in titles:
        ...
        _, _, text_width, text_height = draw.multiline_textbbox(
            (0, 0), wrapped_text, font
        )
        wrapped_title = WrappedTitle(
            wrapped_text=wrapped_text, text_width=text_width, text_height=text_height
        )
...
def wrap_text(
    epd_width: int, epd_height: int, font: ImageFont.FreeTypeFont, text: str
) -> str:
