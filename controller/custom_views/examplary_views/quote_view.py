...
class QuoteView(BaseView):
    ...
    def _epd_change(self, first_call: bool) -> None:
        ...
        quote_font = ImageFont.truetype(
            "/usr/share/fonts/truetype/msttcorefonts/Trebuchet_MS_Bold_Italic.ttf", 16
        )
        ...
        wrapped_quote = wrap_text(
            self.epd.width, self.epd.height, quote_font, self.quote
        )
        wrapped_author = wrap_text(
            self.epd.width, self.epd.height, author_font, self.author
        )

        _, _, _, quote_height = draw.multiline_textbbox(
            (0, 0), wrapped_quote, quote_font
        )

        draw.text((0, 10), str(wrapped_quote), font=quote_font, fill=0)
        draw.text(
            (0, quote_height + 24), f"~ {wrapped_author}", font=author_font, fill=0
        )
