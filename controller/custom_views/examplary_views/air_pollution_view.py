...
class AirPollutionView(BaseView):
    ...
    def _epd_change(self, first_call: bool) -> None:
        ...
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/msttcorefonts/Impact.ttf", 22
        )
        font_large = ImageFont.truetype(
            "/usr/share/fonts/truetype/msttcorefonts/Impact.ttf", 32
        )
        font_awesome = ImageFont.truetype(
            "/usr/share/fonts/truetype/font-awesome/fontawesome-webfont.ttf", 32
        )
        ...
        _, _, aqi_label_width, aqi_label_height = draw.multiline_textbbox(
            (0, 0), aqi_label, font_large
        )
        ...
        titles = [
            f"{key.upper()}: {value}"
            for key, value in self.air_data.items()
            if value is not None and key != "aqi"
        ]
        ...
        for data in wrapped_data:
            draw.text(
                (current_width, current_height), data.wrapped_text, font=font, fill=0
            )
    ...
    def _get_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            if len(data_list) > 0:
                ...
                air_data["no2"] = round(
                    data_list[0].get("components", {}).get("no2"), 2
                )
                air_data["o3"] = round(data_list[0].get("components", {}).get("o3"), 2)
                air_data["so2"] = round(
                    data_list[0].get("components", {}).get("so2"), 2
                )
                air_data["pm2.5"] = round(
                    data_list[0].get("components", {}).get("pm2_5"), 2
                )
                air_data["pm10"] = round(
                    data_list[0].get("components", {}).get("pm10"), 2
                )
                air_data["nh3"] = round(
                    data_list[0].get("components", {}).get("nh3"), 2
                )
            return air_data
