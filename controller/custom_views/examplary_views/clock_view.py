...
class ClockView(BaseView):
    ...
    def _epd_change(self, first_call: bool) -> None:
        ...
        big_font = ImageFont.truetype(
            "/usr/share/fonts/truetype/msttcorefonts/Impact.ttf", self.epd.width // 3
        )
        font = ImageFont.truetype(
            "/usr/share/fonts/truetype/msttcorefonts/Impact.ttf", self.epd.width // 6
        )
        draw.text(
            (self.epd.width // 20, self.epd.height // 50),
            f"{current_time}",
            font=big_font,
            fill=0,
        )
        draw.text(
            (self.epd.width // 20, self.epd.width // 4 + self.epd.height // 20),
            f"----------------\n{current_date}",
            font=font,
            fill=0,
        )
    ...
    def _conditional(self, *args: Any, **kwargs: Any) -> bool:
        ...
        if kwargs["first_call"] or self.current_time_date != (
            current_time,
            current_date,
        ):
